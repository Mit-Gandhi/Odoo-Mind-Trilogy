...
import { Camera, Plus, X, Save, MapPin, Clock, Eye, EyeOff, Mail, Linkedin, Github, Globe } from 'lucide-react';
...
const ProfilePage: React.FC<ProfilePageProps> = ({ onNavigateHome, onNavigateBack, onNavigateMessages }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Redirect to home after saving profile (for both new and existing users)
      onNavigateHome();
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Navbar 
        showBackButton={userProfile?.isProfileComplete}
        onBackClick={userProfile?.isProfileComplete ? onNavigateBack : undefined}
              ...
              <div className="flex-1">
                <div className="mb-4">
                  ...
                  <input
                    ...
                    disabled={!isEditing}
                    className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-50 disabled:text-gray-600"
            ...
            {isEditing && (
              <div className="flex justify-end">
                <button
                  onClick={handleSave}
                  disabled={loading}
                  className="px-8 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-all duration-200 font-medium shadow-sm hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
                >
                  {loading ? (
                    <>
