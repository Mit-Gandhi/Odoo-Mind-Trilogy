...
const RequestsPage: React.FC<RequestsPageProps> = ({ onNavigateHome, onNavigateBack, onNavigateMessages }) => {
  const { user } = useAuth();
  ...
  const handleRequestAction = async (requestId: string, action: 'accepted' | 'rejected') => {
    ...
    try {
      ...
      showSuccess(
        ...
        `The request has been ${actionText} and the other user will be notified.`
      );
    } catch (error) {
                      ...
                      <div key={request.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                        <div className="flex items-start justify-between">
                          <div className="flex items-start space-x-4 flex-1">
                            <img
                              src={request.fromUserPhoto || 'https://images.pexels.com/photos/1239291/pexels-photo-1239291.jpeg?auto=compress&cs=tinysrgb&w=400'}
